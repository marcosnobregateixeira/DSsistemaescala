...
export const Settings: React.FC = () => {
  ...
  const handleTeamMappingChange = (teamName: string, shiftName: string) => {
    const currentMappings = settings.teamMappings || [];
    const updatedMappings = currentMappings.map(m => 
      m.teamName === teamName ? { ...m, shiftName } : m
    );
    // If team not found (should not happen with default init), add it
    if (!updatedMappings.find(m => m.teamName === teamName)) {
      updatedMappings.push({ teamName, shiftName });
    }
    setSettings({ ...settings, teamMappings: updatedMappings });
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-xl border border-gray-200">
        <h3 className="text-lg font-black text-pm-900 mb-6 border-b pb-4 flex items-center uppercase tracking-tighter">
          <Users size={22} className="mr-2 text-pm-700"/> Vínculo de Equipes (Ambulância)
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
              <div className="flex items-start space-x-3">
                 ...
                 <div>
                    <h4 className="font-bold text-blue-900 text-sm uppercase mb-1">Como funciona?</h4>
                    <p className="text-xs text-blue-800 leading-relaxed">
                       Defina aqui qual turma (01 ou 02) cada equipe (ALFA, BRAVO, CHARLIE, DELTA) pertence.
                       Isso afetará a geração automática da escala 2x2.
                    </p>
                 </div>
              </div>
           </div>
           ...
           <div className="space-y-4">
              {(settings.teamMappings || []).map((mapping) => (
                <div key={mapping.teamName} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                  <div className="flex items-center space-x-3">
                    <div className={`w-3 h-3 rounded-full ${
                      mapping.teamName === 'ALFA' ? 'bg-blue-600' :
                      mapping.teamName === 'BRAVO' ? 'bg-green-600' :
                      mapping.teamName === 'CHARLIE' ? 'bg-yellow-500' :
                      'bg-purple-600'
                    }`}></div>
                    <span className="font-bold text-gray-700">{mapping.teamName}</span>
                  </div>
                  <select 
                    value={mapping.shiftName}
                    onChange={(e) => handleTeamMappingChange(mapping.teamName, e.target.value)}
                    className="p-2 border rounded-md text-sm font-medium text-gray-700 focus:ring-2 focus:ring-pm-500 outline-none"
                  >
    ...
    const success = await db.initSupabaseSync(true); // Chamada forçada (true) limpa bypass de erro
    if (success) {
      alert('Sincronização concluída com sucesso! Dados atualizados da nuvem.');
      window.location.reload();
    } else {
      alert('Falha na sincronização.\n\nVerifique:\n1. Se você está logado.\n2. Se há conexão com a internet.\n3. Se o outro computador realmente terminou de salvar os dados na nuvem.');
    }
  }} className="ml-4 text-[10px] bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full uppercase font-bold hover:bg-orange-200 transition-all active:scale-95">Sincronizar Agora</button>
           </h4>
      ...
      {/* SEÇÃO: CONEXÃO SUPABASE (NOVO) */}
      <div className="bg-white p-6 rounded-2xl shadow-xl border border-gray-200">
        <h3 className="text-lg font-black text-pm-900 mb-6 border-b pb-4 flex items-center uppercase tracking-tighter">
          <Cloud size={22} className="mr-2 text-pm-700"/> Sincronização em Nuvem (Supabase)
        </h3>
        
        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6">
           <p className="text-sm text-gray-600 mb-4">
              O sistema está configurado para salvar os dados localmente e tentar sincronizar com o banco de dados Supabase.
              Utilize o botão abaixo para verificar se a conexão com a nuvem está ativa.
           </p>
           ...
           <div className="flex items-center space-x-4">
              <button 
                ...
                className="bg-pm-700 text-white px-4 py-2 rounded-lg font-bold uppercase text-xs hover:bg-pm-800 transition-colors disabled:opacity-50 flex items-center"
              >
                {isTestingSupabase ? <RefreshCw className="animate-spin mr-2" size={16}/> : <Cloud className="mr-2" size={16}/>}
                {isTestingSupabase ? 'Testando...' : 'Testar Conexão'}
              </button>

              {supabaseStatus.message && (
                <div className={`text-xs font-bold px-3 py-2 rounded-lg border ${supabaseStatus.success ? 'bg-green-100 text-green-800 border-green-200' : 'bg-red-100 text-red-800 border-red-200'}`}>
                   {supabaseStatus.success ? <Check size={14} className="inline mr-1"/> : <X size={14} className="inline mr-1"/>}
                   {supabaseStatus.message}
                </div>
              )}
           </div>
